...
// Edge <= 6 10 12 - 13+, firefox <="18" 45+, ie 11, safari 5.1 9+, ios 9.1 throw exceptions when non-strict code (e.g., asp.net 4.5) accesses strict mode arguments.callee.caller (trac-13335). but as of jquery 3.0 (2016), should be common enough that all such attempts are guarded in a try block. "use strict"; var arr="[];" getproto="Object.getPrototypeOf;" slice="arr.slice;" flat="arr.flat" ? function( array ) { return arr.flat.call( ); } : arr.concat.apply( [], }; push="arr.push;" indexof="arr.indexOf;" class2type="{};" tostring="class2type.toString;" hasown="class2type.hasOwnProperty;" fntostring="hasOwn.toString;" objectfunctionstring="fnToString.call(" object support="{};" isfunction="function" isfunction( obj support: chrome some browsers, typeof returns "function" for html <object> elements
		...
		// Support: QtWeb <=3.8.5, 0 1 2369 webkit <="534.34," wkhtmltopdf tool plus for old webkit, typeof returns "function" html collections (e.g., `typeof document.getelementsbytagname("div")="==" "function"`). (gh-4756) return obj="==" && obj.nodetype !="=" "number" obj.item "function"; }; var iswindow="function" iswindow( ) { obj.window; document="window.document;" preservedscriptattributes="{" type: true, src: nonce: nomodule: true function domeval( code, node, doc || document; i, val, script="doc.createElement(" "script" ); script.text="code;" if ( node i in support: firefox 64+, edge 18+ some browsers don't support the "nonce" property on scripts. other hand, just using `getattribute` is not enough as `nonce` attribute reset to an empty string whenever it becomes browsing-context connected. see https: github.com whatwg issues html.spec.whatwg.org #nonce-attributes `node.getattribute` check was added sake of `jquery.globaleval` so that can fake a nonce-containing via object. val="node[" ] node.getattribute node.getattribute( script.setattribute( } doc.head.appendchild( ).parentnode.removechild( totype( null + ""; android only (functionish regexp) "object" ? class2type[ tostring.call( : obj; * global symbol defining this .eslintrc.json would create danger unguarded another place, seems safer define module version="3.6.0" , local copy jquery selector, context object actually initial constructor 'enhanced' need called (just allow error be thrown included) new jquery.fn.init( jquery.fn="jQuery.prototype" = current being used jquery: version, constructor: jquery, default length length: 0, toarray: function() slice.call( }, get nth element matched set or whole clean array get: function( num all elements one from this[ this.length ]; take and push onto stack (returning set) pushstack: elems build ret="jQuery.merge(" this.constructor(), add (as reference) ret.prevobject="this;" newly-formed ret; execute callback every set. each: jquery.each( this, map: this.pushstack( jquery.map( elem, callback.call( elem slice: slice.apply( arguments first: this.eq( last: -1 even: jquery.grep( _elem, % 2; odd: eq: len="this.length," j="+i">= 0 && j < len ? [ this[ j ] ] : [] );
	},
...
var rootjQuery,
	...
	init = jQuery.fn.init = function( selector, context, root ) {
		...
		// Method initial() accepts an alternate rootjQuery
...
// Give the initial function the jQuery prototype for later instantiation
		...
		handler: function( event ) {
			...
			if ( ( event.isTrigger & 1 ) && this[ type ] ) {
				...
				if ( !saved.length ) {
					...
					// Support: IE <=9 0 1 3 7 12 13 2003 1736512 - 11+ focus() and blur() are asynchronous notasync="expectSync(" this, type ); this[ ](); result="dataPriv.get(" if ( saved !="=" || ) { datapriv.set( type, false } else cancel the outer synthetic event event.stopimmediatepropagation(); event.preventdefault(); support: chrome 86+ in chrome, an element having a focusout handler is blurred by clicking outside of it, it invokes synchronously. that calls `.remove()` on element, data cleared, leaving `result` undefined. we need to guard against this. return && result.value; this inner for with bubbling surrogate (focus or blur), assume already propagated from triggering native prevent happening again here. technically gets ordering wrong w.r.t. `.trigger()` (in which propagates *after* non-bubbling initial), but seems less bad than duplication. jquery.event.special[ ] {} ).delegatetype event.stoppropagation(); triggered above, everything now order fire original arguments saved.length ...and capture value: jquery.event.trigger( ie <="9" extend prototype reset above stopimmediatepropagation() jquery.extend( saved[ ], jquery.event.prototype ), saved.slice( abort handling jquery.removeevent="function(" elem, handle "if" needed plain objects elem.removeeventlistener elem.removeeventlistener( }; jquery.event="function(" src, props allow instantiation without 'new' keyword !( instanceof new jquery.event( object src src.type this.originalevent="src;" this.type="src.type;" events up document may have been marked as prevented lower down tree; reflect correct value. this.isdefaultprevented="src.defaultPrevented" src.defaultprevented="==" undefined android only src.returnvalue="==" ? returntrue : returnfalse; create target properties safari should not be text node (#504, #13143) this.target="(" src.target src.target.nodetype="==" src.target.parentnode src.target; this.currenttarget="src.currentTarget;" this.relatedtarget="src.relatedTarget;" put explicitly provided onto timestamp incoming doesn't one this.timestamp="src" src.timestamp date.now(); mark fixed jquery.expando based dom3 specified ecmascript language binding https: www.w3.org tr wd-dom-level-3-events-20030331 ecma-script-binding.html constructor: jquery.event, isdefaultprevented: returnfalse, ispropagationstopped: isimmediatepropagationstopped: issimulated: false, preventdefault: function() var e="this.originalEvent;" !this.issimulated e.preventdefault(); }, stoppropagation: this.ispropagationstopped="returnTrue;" e.stoppropagation(); stopimmediatepropagation: this.isimmediatepropagationstopped="returnTrue;" e.stopimmediatepropagation(); this.stoppropagation(); includes all common including keyevent mouseevent specific jquery.each( altkey: true, bubbles: cancelable: changedtouches: ctrlkey: detail: eventphase: metakey: pagex: pagey: shiftkey: view: "char": code: charcode: key: keycode: button: buttons: clientx: clienty: offsetx: offsety: pointerid: pointertype: screenx: screeny: targettouches: toelement: touches: which: true jquery.event.addprop focus: "focusin", blur: "focusout" function( delegatetype utilize possible so blur focus sequence setup: claim first "focus", ... "blur", leveragenative( expectsync normal processing caller false; trigger: force setup before trigger non-false event-path propagation true; suppress it's being fired leveragenative. _default: delegatetype: mouseenter leave using mouseover out event-time checks delegation works jquery. do same pointerenter pointerleave pointerover pointerout sends too often; see: bugs.chromium.org p chromium issues detail?id="470258" description bug (it existed older versions well). mouseenter: "mouseover", mouseleave: "mouseout", pointerenter: "pointerover", pointerleave: "pointerout" orig, fix orig fix, bindtype: handle: ret, related="event.relatedTarget," handleobj="event.handleObj;" call target. nb: no relatedtarget mouse left entered browser window !related !jquery.contains( target, event.type="handleObj.origType;" ret="handleObj.handler.apply(" ret; jquery.fn.extend( on: types, selector, data, fn on( one: fn, off: handleobj, type; types types.preventdefault types.handleobj dispatched jquery( types.delegatetarget ).off( handleobj.namespace handleobj.origtype + "." handleobj.origtype, handleobj.selector, handleobj.handler this; typeof "object" types-object [, selector] this.off( types[ selector="==" "function" fn] this.each( jquery.event.remove( 11, edge regex groups here causes severe slowdowns. see connect.microsoft.com feedback details rnoinnerhtml="/<script|<style|<link/i," checked="checked" rchecked="/checked\s*(?:[^=]|=\s*.checked.)/i," rcleanscript="/^\s*<!(?:\[CDATA\[|--)|(?:\]\]|--)">\s*$/g;
...
jQuery.parseHTML = function( data, context, keepScripts ) {
	...
	if ( !context ) {
		...
		if ( support.createHTMLDocument ) {
			...
			// Set the initial href for the created document
...
// Support: Android </=11></div></=9></=44></=10></=9></=4.3></=9></=54></=35></=4.0></=9></=42></=9></=9></=9></tbody></=4.1></=43></=9></=></=></=4.3></object></"></)></tag></\></537.32></9,></9></8></9></option></29,></=8></4.0></=11+></=4.0></=3.8.5,></=>